...
const validateAndNormalise = config => {
    ...
    const host = utils_1.sanitizeStringInput(postgresInputConfig.host);
    const port = utils_1.sanitizeNumberInput(postgresInputConfig.port);
    const user = utils_1.sanitizeStringInput(postgresInputConfig.user);
    if (user === undefined) {
        throw error_1.generateError(
            error_1.AuthError.GENERAL_ERROR,
            new Error("postgres config error. user not passed"),
            false
        );
    }
    const password = utils_1.sanitizeStringInput(postgresInputConfig.password);
    if (password === undefined) {
        throw error_1.generateError(
            error_1.AuthError.GENERAL_ERROR,
            new Error("postgres config error. password not passed"),
            false
        );
    }
    const database = utils_1.sanitizeStringInput(postgresInputConfig.database);
